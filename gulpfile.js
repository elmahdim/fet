...
var uniqueID = new Date().toJSON().replace(/[^0-9]/g, "");
var buildDir = 'build-' + uniqueID + '/';
...
// ============================================
gulp.task('theme', ['importTheme'], function (cb) {
    return gulp.src(defaults.themes.src)
        .pipe(gulp.dest(defaults.themes.dest + argv.name));
});

gulp.task('importTheme', function(cb){
  fileSystem.writeFile('src/sass/main.scss', '@import "themes/'+argv.name+'/bootstrap";', cb);
});
